// First let's parse the JSONL data and create both required files
const meetups = `{"link":"https://www.meetup.com/hyderabad-explorers/","name":"Hyderabad Explorers- New in the city","location":"Hyderabad, IN"}
...
{"link":"https://www.meetup.com/flutterjaipur/","name":"Flutter Jaipur","location":"Jaipur, IN"}`.split('\n').filter(line => line.trim());

// Create the index
const index = {};
let position = 0;

meetups.forEach(line => {
  const entry = JSON.parse(line);
  const name = entry.name.toLowerCase();
  index[name] = [position, position + line.length];
  position += line.length + 1; // +1 for newline
});

console.log("meetups_index.json:");
